...
- **Compile-time Format String Hardening**: Printing macros in `vibe_io.h` and `vibe_log.h` use string literal concatenation (`"" fmt`) to force the first argument to be a literal. This prevents format string injection vulnerabilities at compile-time (v1.5.1).
